...
from VRP.quantum.BQM_based.route_activation_solver import (
    RouteActivationSolver as RAS_bqm,
)
from VRP.quantum.BQM_based.dbscan_solver import DBSCANSolver as DBSS_bqm
from VRP.quantum.BQM_based.solution_partition_solver import (
    SolutionPartitionSolver as SPS_bqm,
)
...
class VRPSolver:
    ...
    def solve_vrp(self):
        ...
        solver = {
            ...
            "BQM": {"fqs": FQS_bqm, "ras": RAS_bqm, "dbss": DBSS_bqm, "sps": SPS_bqm},
        }[model][self.params["solver"]]

        self.solver = solver(self.n, self.m, self.cost, xc=self.xc, yc=self.yc)
        if model == "BQM":
            return self.solver.solve(solver="neal")

        return self.solver.solve(time_limit=time_limit)
...
def compare_solvers(n, m, cost, xc, yc, **params):
    ...
    avg_min_costs = [
        sum_min_costs[i] / n_iter if sum_min_costs[i] != 0 else None
        for i in range(len(solversList))
    ]
    avg_runtimes = [sum_runtimes[i] / n_iter for i in range(len(solversList))]
    approximation_ratios = [
        avg_min_costs[i] / exact_min_cost if avg_min_costs[i] is not None else None
        for i in range(len(solversList))
    ]

    comparison_table = {
        solversList[i].__name__: {
            "avg_min_cost": avg_min_costs[i],
            "avg_runtime": avg_runtimes[i],
            "num_vars": num_vars[i],
            "approximation_ratio": approximation_ratios[i],
        }
        for i in range(len(solversList))
    }
...
def custom_routing_instance(n):
    ...
    custom_coords = [
        (
            [0, 9.68397635, -21.91415037, 8.33057986, 2.96044684, -20.74446885],
            [0, -5.37904717, -0.14612363, -8.39254627, -7.88307738, -6.71815846],
        ),
        (
            [0, -14.62374849, 0.64433429, -22.06068663, -24.17317981, 12.94189313],
            [0, 7.79525625, -2.17971286, -9.13602211, 19.52634242, 17.35703871],
        ),
        (
            [0, -20.547731, 0.21868276, 22.94555096, -21.60056947, -12.77923658],
            [0, -21.68070314, 3.85064948, 10.24842767, 5.33432128, -23.79246391],
        ),
        (
            [0, -23.95905211, -1.81608017, 5.17390474, 14.4451113, 7.92617635],
            [0, 11.01641074, -11.25120004, -23.69568939, 2.42716362, 11.41399686],
        ),
        (
            [0, 15.56202638, -8.1563987, -14.75819874, -16.40754363, 0.99306327],
            [0, 2.10506766, 14.74870063, 4.10311324, 20.76978131, -9.60709729],
        ),
        (
            [0, 17.53090505, -10.37294117, -13.84557367, -15.11426717, -6.10549414],
            [0, -18.07329384, -24.45858156, -20.871561, 22.97596238, 11.09926968],
        ),
        (
            [0, 17.39440017, -24.6850199, -19.22633574, -21.8862367, 10.20874553],
            [0, -2.42883951, -23.66801217, 9.19644146, 15.24107506, -24.45443753],
        ),
        (
            [0, -24.23999789, 24.44285518, 8.72046533, -2.01580788, -6.19607689],
            [0, -11.48069452, -17.60094231, 2.67137847, -13.23332478, -8.5483955],
        ),
        (
            [0, -8.72775711, 19.45745275, -4.44703119, -16.1182592, -12.93482615],
            [0, 19.96848055, 2.78356029, -3.79202314, -24.46107761, 23.34860146],
        ),
        (
            [0, 12.79365238, -12.45608602, 10.29188926, -6.11647882, 11.14503212],
            [0, 5.35068747, 8.78709806, -18.40157431, 23.6464964, -13.89138151],
        ),
    ]
